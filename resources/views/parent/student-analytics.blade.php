...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="mb-8">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="mt-2 text-sm text-gray-600">Compare your child's assessment performance vs term results for each subject</p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-4 mb-6">
        <form method="GET" action="{{ route('parent.student-analytics') }}" id="termFilterForm" class="flex items-center space-x-4">
            @if($selectedStudentId)
            <input type="hidden" name="student_id" value="{{ $selectedStudentId }}">
            @endif
            <div class="flex items-center space-x-2">
                <label class="text-sm font-medium text-gray-700">Year:</label>
                <select name="year" id="yearFilter" class="px-3 py-1.5 text-sm border border-gray-300 rounded-lg bg-white shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                    @php
            ...
            <div class="flex items-center space-x-2">
                ...
                <select name="term" id="termFilter" class="px-3 py-1.5 text-sm border border-gray-300 rounded-lg bg-white shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
    ...
    <!-- CHILDREN CARDS VIEW -->
    <div class="mb-6">
        <h2 class="text-lg font-semibold text-gray-800 mb-4">Select a Child</h2>
    </div>
    
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
        @forelse($children as $child)
        <a href="{{ route('parent.student-analytics', ['student_id' => $child->id, 'year' => $selectedYear, 'term' => $selectedTerm]) }}" 
           class="bg-white rounded-xl shadow-sm border border-gray-200 hover:shadow-lg hover:border-green-300 transition-all transform hover:-translate-y-1 p-6">
            <div class="flex items-center space-x-4">
                <div class="w-14 h-14 bg-gradient-to-br from-green-400 to-emerald-600 rounded-full flex items-center justify-center flex-shrink-0">
                    <span class="text-xl font-bold text-white">{{ substr($child->user->name ?? 'S', 0, 1) }}</span>
                </div>
                <div class="min-w-0">
                    <h3 class="text-lg font-semibold text-gray-900 truncate">{{ $child->user->name ?? 'N/A' }}</h3>
                    <p class="text-sm text-gray-500">{{ $child->class->class_name ?? 'N/A' }}</p>
                    <p class="text-xs text-gray-400">Roll: {{ $child->roll_number }}</p>
                </div>
            </div>
            <div class="mt-4 flex items-center justify-between">
                <span class="text-xs text-gray-400">Click to view analytics</span>
        ...
        <div class="col-span-full bg-yellow-50 border border-yellow-200 rounded-xl p-8 text-center">
            ...
            <p class="text-yellow-700 font-medium">No children found</p>
            <p class="text-yellow-600 text-sm mt-1">Please contact the school administration.</p>
        </div>
        @endforelse
    </div>
    ...
    <div class="mb-6">
        <a href="{{ route('parent.student-analytics', ['year' => $selectedYear, 'term' => $selectedTerm]) }}" 
