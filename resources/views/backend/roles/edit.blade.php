    ...
    <div class="mb-8">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="mt-2 text-sm text-gray-600">Update role details and permissions</p>
            </div>
                ...
                <div class="mb-6">
                    ...
                    <div class="space-y-3">
                        @foreach($groupedPermissions as $category => $items)
                            @php
                                $categoryKeys = array_keys($items);
                                $checkedCount = count(array_intersect($categoryKeys, $rolePermissions));
                                $allChecked = $checkedCount === count($items);
                            @endphp
                            <div x-data="{ open: {{ $checkedCount > 0 ? 'true' : 'false' }}, selectAll: {{ $allChecked ? 'true' : 'false' }} }" class="border border-gray-200 rounded-lg overflow-hidden">
                                <!-- Category Header -->
                                <div class="bg-gray-50 px-4 py-3 flex items-center justify-between cursor-pointer hover:bg-gray-100 transition-colors" @click="open = !open">
                                    <div class="flex items-center space-x-3">
                                        ...
                                        <span class="text-xs text-gray-500 bg-gray-200 px-2 py-0.5 rounded-full">{{ $checkedCount }}/{{ count($items) }}</span>
                                    </div>
                                        ...
                                        @foreach($items as $permissionKey => $permissionLabel)
                                            <label class="relative flex items-center p-2 rounded-lg border border-gray-200 hover:border-blue-300 hover:bg-blue-50 cursor-pointer transition-all group">
                                                <input type="checkbox" name="selectedpermissions[]" value="{{ $permissionKey }}" 
                                                    {{ in_array($permissionKey, $rolePermissions) ? 'checked' : '' }}
                                                    class="h-4 w-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500">
