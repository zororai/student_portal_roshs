...
<div class="container-fluid px-4 py-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-8">
        <div>
            ...
            <p class="text-gray-500 mt-1 ml-13">View your child's class schedule</p>
        </div>
        <div class="mt-4 md:mt-0 flex items-center gap-3">
            @if($children->count() > 1)
                <form method="GET" action="{{ route('parent.timetable') }}" class="flex items-center gap-2">
                    <select name="child_id" onchange="this.form.submit()" class="px-4 py-2 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500">
                        @foreach($children as $child)
                            <option value="{{ $child->id }}" {{ ($selectedChild && $selectedChild->id == $child->id) ? 'selected' : '' }}>
                                {{ $child->user->name ?? $child->name }}
                            </option>
                        @endforeach
                    </select>
                </form>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-4 mb-6">
            <div class="flex items-center gap-4">
                <div class="w-12 h-12 bg-gradient-to-br from-emerald-400 to-teal-500 rounded-full flex items-center justify-center text-white font-bold text-lg">
                    {{ strtoupper(substr($selectedChild->user->name ?? 'S', 0, 1)) }}
                </div>
                <div>
                    <h2 class="text-lg font-semibold text-gray-800">{{ $selectedChild->user->name ?? 'Student' }}</h2>
                    <p class="text-sm text-gray-500">{{ $class->class_name }} - Form {{ $class->class_numeric }}</p>
                </div>
            </div>
        </div>
