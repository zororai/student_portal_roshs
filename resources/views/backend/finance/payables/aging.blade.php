...
<div class="flex justify-between items-center mb-6">
    <div>
        <h1 class="text-2xl font-bold text-gray-800">Accounts Payable Aging Report</h1>
        <p class="text-gray-500 text-sm">Track outstanding payables by aging period</p>
    </div>
    <div class="flex gap-2">
        ...
        <a href="{{ route('finance.payables.index') }}" class="bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-lg shadow-md transition duration-200">
            ← Back to Dashboard
        </a>
    </div>
</div>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                @forelse($agingData as $data)
                    <tr class="hover:bg-gray-50">
                        <td class="px-6 py-4">
                            <div class="text-sm font-medium text-gray-900">{{ $data['supplier']->name }}</div>
                            <div class="text-xs text-gray-500">{{ $data['supplier']->email ?? 'No email' }}</div>
                        </td>
                    ...
                    <tr>
                        <td colspan="6" class="px-6 py-12 text-center">
                            <div class="flex flex-col items-center justify-center text-gray-400">
                                ...
                                <p class="text-lg font-medium">No outstanding payables!</p>
                                <p class="text-sm">All supplier invoices have been paid.</p>
                            </div>
                        </td>
                    </tr>
                @endforelse
